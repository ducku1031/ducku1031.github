...
|RD|1|Recursion Desired. 질의에 대한 응답 결과를 얻을 때까지 DNS 서버가 반복|쿼리2를 수행하도록 요청함을 의미하며 요청 메시지에서 사용됨|
|RA|1|Recursion Available. 해당 DNS 서버가 DNS 클라이언트의 반복 쿼리 요청을 |수행할 수 있음을 의미하며 응답 메시지에서 사용됨|
|Z (Zero)|3|예약 필드로 000으로 지정됨|rcode|4|Return code. 0=에러 없음, 1=양식오류, 2=서버실패, 3=이름에러, 4=구현되지 |않음, 5=거절, 6-15=추후 사용|